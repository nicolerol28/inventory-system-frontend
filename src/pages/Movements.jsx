          ...
          {movements?.content?.length > 0 && (
            <button
              ...
              onClick={async () => {
                setExporting(true);
                try {
                  const all = dateFilterActive && fromDate && toDate
                    ? await getMovementsByDateRange(activeWarehouseId, fromDate + ":00", toDate + ":00", 0, 10000)
                    : await getMovementsByWarehouse(activeWarehouseId, 0, 10000, filterType);
                  const rows = (all.content ?? []).map((m) => ({
                    ...m,
                    productName: products?.content?.find(p => p.id === m.productId)?.name ?? `#${m.productId}`,
                    movementType: MOVEMENT_TYPE_LABELS[m.movementType] ?? m.movementType,
                  }));
                  await exportToExcel(rows, [
                    ...
                    { key: "quantityAfter", header: "Después" },
