...
import { getSuppliers, createSupplier, updateSupplier, deactivateSupplier } from "../api/suppliers";
...
function SupplierForm({ supplier, onSubmit, onClose, isLoading }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div>
        ...
        <input name="contact" value={form.contact} onChange={handleChange} className={inputClass} />
      </div>
      <div>
...
export function Suppliers() {
  ...
  const createMutation = useMutation({
    ...
    onError: (error) => {
      setErrorMsg(error.response?.data?.message ?? "Error al crear el proveedor.");
    },
  });
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      setErrorMsg(error.response?.data?.message ?? "Error al actualizar el proveedor.");
    },
  });
  ...
  const deactivateMutation = useMutation({
    ...
    onError: (error) => {
      alert(error.response?.data?.message ?? "Error al desactivar el proveedor.");
    },
});
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold text-gray-900 dark:text-white">Proveedores</h1>
          <p className="text-xs text-gray-400 dark:text-gray-600 mt-0.5">
            {suppliers?.totalElements ?? 0} proveedores en total
          </p>
        </div>
        {isAdmin && (
          <button
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl transition-colors duration-300">
        ...
        {isLoading ? (
          ...
          <>
            <table className="w-full">
              <thead>
                <tr className="text-left">
                  ...
                  <th className="px-6 py-3 text-[10px] text-gray-400 dark:text-gray-600 uppercase tracking-widest font-medium">Contacto</th>
                  <th className="px-6 py-3 text-[10px] text-gray-400 dark:text-gray-600 uppercase tracking-widest font-medium">Teléfono</th>
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                {suppliers?.content?.map((s) => (
                  <tr key={s.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                    <td className="px-6 py-3 text-sm font-medium text-gray-700 dark:text-gray-300">{s.name}</td>
                    <td className="px-6 py-3 text-sm text-gray-500 dark:text-gray-400">{s.contact ?? "—"}</td>
                    <td className="px-6 py-3 text-sm text-gray-500 dark:text-gray-400">{s.phone ?? "—"}</td>
                    <td className="px-6 py-3"><Badge active={s.active} /></td>
                    {isAdmin && (
            ...
            <Pagination page={page} totalPages={suppliers?.totalPages ?? 0} onPageChange={setPage} />
          </>
        )}
      </div>
